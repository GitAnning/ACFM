...
class dict_dataset(torch.utils.data.Dataset):
    def __init__(self,imagery_dict,dataset_dict,path,mode,transform,percent=[0.7,0.1,0.2],shuffle=True,load_mode=False,load_mode_reshape_size=(600,600)):
    ...
    def __getitem__(self,index):
        ...
        imagery=self.imagery_dict[key_name]
        ...
        if isinstance(label,list):
            label.insert(0,imagery)
            label.insert(0, image)
            return tuple(label)
        else:
            return image,imagery,label
...
class three_set_dataset(dict_dataset):
    def __init__(self,imagery_dict,dataset_dict,path,mode,transform,load_mode=True,load_mode_reshape_size=(600,600)):
        self.imagery_dict=imagery_dict
        self.path=path
        self.mode=mode
        self.transform=transform[self.mode]
...
class key_dataset(dict_dataset):
    ...
    def __init__(self,imagery_dict,dataset_dict,path,mode,transform,target_file_path=[],shuffle=True,load_mode=False,load_mode_reshape_size=(600,600)):
        self.imagery_dict = imagery_dict
        self.dataset_dict=dataset_dict
        self.path=path
        self.mode=mode
